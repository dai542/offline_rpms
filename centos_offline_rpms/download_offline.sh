...
yum -y install dnf createrepo_c yum-utils createrepo --nogpgcheck
...
# Configure YUM for better performance
...
while IFS= read -r line; do
    ...
    if [[ $package == @* ]]; then
        ...
        yum groupinstall --downloadonly --downloaddir="$DOWNLOAD_DIR/rpms" -y "$group" >> "$LOG_FILE" 2>&1
    else
        if [[ $package == @* ]]; then
            group="${package#@}"
            echo "Downloading group: $group"
            ...
            yumdownloader --resolve --destdir="$DOWNLOAD_DIR/rpms" "$package" >> "$LOG_FILE" 2>&1
        fi
    fi
done < "$PACKAGE_LIST"
...
check_dependencies() {
    ...
    if [ -n "$missing_deps" ]; then
        ...
        echo "$missing_deps" | xargs -r yumdownloader --resolve --destdir="$DOWNLOAD_DIR/rpms"
        return 1
    else
...
rpm -Uvh "$MOUNT_POINT"/rpms/dnf*.rpm --nodeps
...
while IFS= read -r line; do
    ...
    if [[ \$package == @* ]]; then
        ...
        yum groupinstall -y --disablerepo=* --enablerepo=local "\$group"
    else
        # Install individual package
        echo "Installing package: \$package"
        yum install -y --disablerepo=* --enablerepo=local "\$package"
    fi
